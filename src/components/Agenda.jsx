...
export default function Agenda({ onSelectContact }) {
  ...
  const saveContacts = (updatedContacts) => {
    ...
    if (isGoogleSynced) {
      // Futura integração com o appDataFolder do Google Drive
  ...
  const handleDeleteContact = (id, e) => {
    // impede que o clique no botão de lixeira acione a seleção do contato
    e.stopPropagation(); 
  ...
  return (
    <div className="flex flex-col justify-between h-full space-y-4">
      ...
      {/* Alerta de Segurança */}
      <div className="border border-acidGreenDim bg-terminalGray/50 p-3 rounded text-[11px] text-acidGreenDim flex gap-2 items-start shrink-0">
      ...
      {!showAddForm && (
        <button
          ...
          className="w-full py-2 border border-dashed border-acidGreen hover:border-solid hover:bg-acidGreenDim text-acidGreen font-bold rounded text-xs tracking-widest uppercase transition-all duration-200 flex items-center justify-center gap-1.5 shrink-0"
        >
      ...
      {showAddForm && (
        <form onSubmit={handleAddContact} className="border border-acidGreen bg-terminalGray p-4 rounded space-y-3 shadow-green-glow shrink-0">
      ...
      {/* Lista de Contatos Consolidada e Funcional */}
      <div className="flex-1 overflow-y-auto max-h-[45vh] space-y-2 pr-1">
        {contacts.length === 0 ? (
          ...
          contacts.map((contact) => (
            <div 
              key={contact.id}
              onClick={() => onSelectContact && onSelectContact(contact.key)}
              className="flex items-center justify-between border border-borderGray bg-terminalGray/30 p-3 rounded hover:border-acidGreen transition-all group cursor-pointer active:scale-[0.98]"
            >
              <div className="flex flex-col">
                <span className="text-xs font-bold tracking-wider text-acidGreen group-hover:text-white transition-colors">
                  {contact.name}
                </span>
                <span className="text-[10px] text-acidGreenDim font-mono tracking-widest mt-0.5">
                  {contact.key}
                </span>
              </div>
              <button
